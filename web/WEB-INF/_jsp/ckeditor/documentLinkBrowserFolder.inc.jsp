...
<li class="open">
    ...
    <ul>
        ...
        <li>
            <div class="treeline">
                <a id="<%=document.getId()%>" href="" onclick="return ckLinkCallback('<%=document.getStaticURL()%>?download=true');">
                    <%=$H(document.getDisplayName())%>
                </a>
                <a class="fa fa-eye" title="<%=$SH("_download")%>" href="<%=document.getStaticURL()%>?download=true"> </a>
            </div>
        </li>
        <%}
        }
        for (PageData subFolder : contentData.getChildren()) {
